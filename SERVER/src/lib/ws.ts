...
const WebsocketConnection = async (websock: WebSocket.Server) => {
    ...
    websock.on('connection', (ws: WebSocket) => {
        ws.on('message', (message: string) => {
            const jsonValidation = IsJsonSring(message);
            if (!jsonValidation) {
    ...
    const onProduce = async (event: any, ws: WebSocket, websocket:WebSocket.Server) => {
        ...
        broadcast(websocket, 'newProuder', 'new user');
    }
...
const onCreateProducerTransport = async (event: string, ws: WebSocket) => {
    try{
        ...
        send(ws, "producerTransportCreate", params);
    } catch (error) {
        send(ws, "error", error);
    }
}
const IsJsonSring = (str: string) => {
    try{
        JSON.parse(str);
    } catch (error) {
        return false;
