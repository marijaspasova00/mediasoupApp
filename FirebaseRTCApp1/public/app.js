...
async function createRoom() {
  ...
  document.querySelector(
    "#currentRoom"
  ).innerText = `Current room is ${roomId}`;
  ...
  peerConnection.addEventListener("icecandidate", (event) => {
    if (event.candidate) {
      ...
      roomRef.collection("calleeCandidates").add(iceCandidate.toJSON());
    }
  });
  ...
  peerConnection.addEventListener("icecandidate", (event) => {
    if (event.candidate) {
      ...
      // Update the Firestore document with the remote ICE candidate
      roomRef.collection("calleeCandidates").add(iceCandidate.toJSON());
    }
  });
